...
import { Plus, Search, Edit2, Trash2, ArrowUpCircle, X, ChevronUp, ChevronDown, RotateCcw, Image as ImageIcon, Eye } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          Hapus <strong className="text-gray-900 dark:text-white">"{name}"</strong>? Tidak dapat dibatalkan.
        </p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Batal</button>
...
export function TransaksiKeluarPage() {
  ...
  const keluarTx = transactions.filter(t => t.type === 'keluar');
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [keluarTx, search, dateFrom, dateTo, catFilter, sortKey, sortDir]);

  const totalNominalAll = useMemo(() => keluarTx.reduce((s, t) => s + t.amount, 0), [keluarTx]);
  ...
  const SortIcon = ({ k }: { k: SortKey }) =>
    ...
    sortDir === 'asc' ? <ChevronUp className="w-3 h-3 text-rose-500" /> : <ChevronDown className="w-3 h-3 text-rose-500" />;

  const resetFilter = () => { setSearch(''); setDateFrom(''); setDateTo(''); setCatFilter(''); setPage(1); };

  const openAdd = () => {
    setEditId(null);
    setForm({ ...emptyForm, categoryId: keluarCategories[0]?.id || '' });
    setFormErrors({});
    setReceiptFile(null);
    setModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editId) {
      ...
      toast.success('Transaksi keluar berhasil diperbarui');
    } else {
      addTransaction(fd);
  ...
  const inputClass = (field: string) =>
    `w-full px-4 py-2.5 rounded-xl border text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 outline-none transition-all ${
      formErrors[field] ? 'border-rose-400 focus:ring-2 focus:ring-rose-100' : 'border-gray-200 dark:border-gray-700 focus:border-rose-400 focus:ring-2 focus:ring-rose-100 dark:focus:ring-rose-900/30'
    }`;
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-gray-900 dark:text-white">Transaksi Keluar</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">{keluarTx.length} total transaksi pengeluaran</p>
          </div>
        </div>
        <button onClick={openAdd}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white text-sm shadow-lg shadow-rose-500/25 hover:-translate-y-0.5 transition-all duration-200">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative min-w-48 flex-1">
            ...
            <input type="text" placeholder="Cari transaksi..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:border-rose-400 transition-colors" />
          </div>
          <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPage(1); }}
            className="border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:border-rose-400 transition-colors" />
          <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setPage(1); }}
            className="border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:border-rose-400 transition-colors" />
          <select value={catFilter} onChange={e => { setCatFilter(e.target.value); setPage(1); }}
            className="border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:border-rose-400 transition-colors">
            <option value="">Semua Kategori</option>
            {keluarCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-rose-50 dark:bg-rose-900/20 rounded-2xl p-4 border border-rose-100 dark:border-rose-800/40">
          <p className="text-xs text-rose-600 dark:text-rose-400 mb-1">Total Transaksi</p>
          <p className="text-rose-700 dark:text-rose-300" style={{ fontWeight: 700 }}>{keluarTx.length} transaksi</p>
        </div>
        <div className="bg-rose-50 dark:bg-rose-900/20 rounded-2xl p-4 border border-rose-100 dark:border-rose-800/40">
          <p className="text-xs text-rose-600 dark:text-rose-400 mb-1">Total Nominal</p>
          <p className="text-rose-700 dark:text-rose-300" style={{ fontWeight: 700, fontSize: '0.9rem' }}>{fmt(totalNominalAll)}</p>
        </div>
        <div className="hidden sm:block bg-rose-50 dark:bg-rose-900/20 rounded-2xl p-4 border border-rose-100 dark:border-rose-800/40">
          <p className="text-xs text-rose-600 dark:text-rose-400 mb-1">Total Nominal Halaman Ini</p>
          <p className="text-rose-700 dark:text-rose-300" style={{ fontWeight: 700, fontSize: '0.9rem' }}>{fmt(pageNominal)}</p>
        </div>
      </div>
              ...
              {paginated.length === 0 ? (
                <tr><td colSpan={6} className="py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm">Tidak ada transaksi pengeluaran</p>
                    <button onClick={openAdd} className="text-rose-600 dark:text-rose-400 text-sm hover:underline">+ Tambah pengeluaran pertama</button>
                  </div>
                </td></tr>
              ) : paginated.map((t, i) => {
                ...
                return (
                  <motion.tr key={t.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-rose-50/30 dark:hover:bg-rose-900/10 transition-colors group">
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-900 dark:text-white">{t.name}</td>
                    <td className="px-5 py-3.5">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400">{cat?.name || '-'}</span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm text-rose-600 dark:text-rose-400" style={{ fontWeight: 600 }}>-{fmt(t.amount)}</span>
                    </td>
                    ...
                    <td className="px-5 py-3.5 text-center">
                      {t.receipt_url ? (
                        <a href={t.receipt_url} target="_blank" rel="noopener noreferrer" className="group/img inline-block relative">
                          <img src={t.receipt_url} alt="Bukti" className="w-12 h-9 object-cover rounded-lg border border-gray-100 dark:border-gray-800 shadow-sm group-hover/img:ring-2 group-hover/img:ring-rose-500/30 transition-all" />
                     ...
                     <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(t)} className="p-2 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 text-gray-400 hover:text-blue-600 transition-all"><Edit2 className="w-3.5 h-3.5" /></button>
                        <button onClick={() => { setDeleteId(t.id); setConfirmOpen(true); }} className="p-2 rounded-lg hover:bg-rose-100 dark:hover:bg-rose-900/30 text-gray-400 hover:text-rose-600 transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </motion.tr>
                );
        ...
        <div className="sm:hidden divide-y divide-gray-100 dark:divide-gray-800">
          {paginated.length === 0 ? (
            <div className="py-16 flex flex-col items-center gap-3">
              <ArrowUpCircle className="w-10 h-10 text-rose-300" />
            ...
            return (
              <div key={t.id} className="flex items-start gap-3 p-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                <div className="w-9 h-9 rounded-xl bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center text-rose-600 shrink-0">↑</div>
                ...
                <div className="flex flex-col items-end gap-1">
                  <p className="text-sm text-rose-600 dark:text-rose-400" style={{ fontWeight: 600 }}>-{fmt(t.amount)}</p>
                  <div className="flex gap-1">
                    {t.receipt_url && <a href={t.receipt_url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg hover:bg-rose-100 text-gray-400 hover:text-rose-600 transition-all"><ImageIcon className="w-3.5 h-3.5" /></a>}
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-1">
                ...
                {[...Array(Math.min(totalPages, 5))].map((_, i) => {
                  ...
                  return <button key={p} onClick={() => setPage(p)} className={`w-8 h-8 rounded-lg text-xs transition-colors ${page === p ? 'bg-rose-500 text-white' : 'border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>{p}</button>;
                })}
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-gray-900 dark:text-white">{editId ? 'Edit' : 'Tambah'} Pengeluaran</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400">Transaksi keluar</p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Contoh: Pembelian Stok" className={inputClass('name')} />
            ...
            <div>
              ...
              {form.amount && !isNaN(Number(form.amount)) && (
                <p className="mt-1 text-xs text-rose-600 dark:text-rose-400">{fmt(Number(form.amount))}</p>
              )}
            ...
            <div>
              ...
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Tambahkan catatan..." rows={2}
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100 dark:focus:ring-rose-900/30 transition-all resize-none" />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="file" accept="image/*" onChange={e => setReceiptFile(e.target.files?.[0] || null)}
                  className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-rose-50 file:text-rose-700 hover:file:bg-rose-100 dark:file:bg-rose-900/30 dark:file:text-rose-400" />
                {editId && transactions.find(t => t.id === editId)?.receipt_url && (
                  <a href={transactions.find(t => t.id === editId)?.receipt_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-rose-600 hover:underline shrink-0">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button onClick={handleSave} className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white text-sm transition-all shadow-lg shadow-rose-500/20">
